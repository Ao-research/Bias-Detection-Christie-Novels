...
configfile = open(args.config, "r", encoding='utf-8')

basicout = open(args.basicout_path, "a", encoding='utf-8')
richout = open(args.richout_path, "a", encoding='utf-8')
...
def get_adj_SO(index):
    ...
    if language == "English":
        ...
        if not use_comparatives and (get_word(text[index - 1]) in comparatives):
            return 0
        if not use_superlatives and ((get_word(text[index - 1]) in comparatives and get_tag(text[index - 2]) == "DA") or (JJ in ["mejor", "pésimo"] and get_tag(text[index - 2]) == "DA")):
            return 0
    ...
    if JJ in not_wanted_adj:
        ...
        if int_modifier != 0:
            ...
            elif (language == "Spanish" and (get_word(text[index - 1]) in comparatives and get_tag(text[index - 2]) == "DA") or (JJ in ["mejor", "pésimo"] and get_tag(text[index - 2]) == "DA")):
                output.append("(SUPERLATIVE)")
